...
{
    public partial class InsurerPolicyTransactions : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    hdPartnerID.Value = U.CryptorEngine.GenericEncrypt(objUser.iPartner_Id.ToString(), true);
                    GetFormFields();
                }
            }
        }
        ...
        private void NonPaymentForm()
        {
            ...
            if (status == Convert.ToInt32(C.Common.Common.Policy_Status.Active) && status != 0)
            {
                ...
                var rdr = pro.Get_Individual_Policy_Details(Convert.ToInt32(U.CryptorEngine.GenericDecrypt(hdPartnerID.Value, true)), txtPolicy_Number.Text);
        ...
        private void ResumeCoverForm()
        {
            ...
            catch (Exception ex)
            {
                ...
                eL.LogErrorInDB(ex, "InsurerPolicyTransactions", "ResumeCoverForm");
        ...
        {
            P.Policy_Provider pro = new P.Policy_Provider();
            int polId = pro.Get_Policy_Id(Convert.ToInt32(U.CryptorEngine.GenericDecrypt(hdPartnerID.Value, true)), txtPolicy_Number.Text);
        ...
        protected void btnSaveNonPaymnet_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                U.ErrorLogger eL = new U.ErrorLogger();
                eL.LogErrorInDB(ex, "Insurer_PolicyTransactions_PolicyNonPayment", "btnSaveNonPaymnet_Click");
            }
        }
        protected void btnConfirmCoverResume_Click(object sender, EventArgs e)
        {
            P.Policy_Provider pro = new P.Policy_Provider();
            bool iPolicy_Transaction_Id = pro.Update_Policy_Resume_Cover(Convert.ToInt32(U.CryptorEngine.GenericDecrypt(hdPolicyId.Value, true)));
