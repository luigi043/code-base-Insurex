...
{
    public partial class AllAssets : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    //GetAllAssets(objUser.iPartner_Id, ViewState["SortField"].ToString(), "All");
        ...
        private void GetAllAssets(int iPartner_Id, string sortField, string assetTypeFilter)
        {
            ...
            try
            {
                ...
                var sortItem = typeof(C.AssetTypes.AllFinancerAssets).GetProperty(sort);
                var assetItem = typeof(C.AssetTypes.AllFinancerAssets).GetProperty(asset);
                ...
                List<C.AssetTypes.AllFinancerAssets> AllFinancerAssetsByType = new List<C.AssetTypes.AllFinancerAssets>();

                P.Report_Provider frmF = new P.Report_Provider();
                AllFinancerAssetsL = frmF.Get_All_Assets_Financer(iPartner_Id);

                if (assetTypeFilter != "All")
                {
                    AllFinancerAssetsByType = AllFinancerAssetsL.Where(x => x.iAsset_Type_Id == Convert.ToInt32(ddlAsset_type.SelectedValue)).ToList();
                }
                else
                {
                    AllFinancerAssetsByType = AllFinancerAssetsL.ToList();
                }



                if (AllFinancerAssetsByType.Count > 0)
                {
                    if (ViewState["SortOrder"].ToString() == "ASC")
                    {
                        rptUninsuredAssets.DataSource = AllFinancerAssetsByType.OrderBy(x => sortItem.GetValue(x, null)).ToList();

                    }
                    else
                    {
                        rptUninsuredAssets.DataSource = AllFinancerAssetsByType.OrderByDescending(x => sortItem.GetValue(x, null)).ToList();
        ...
        protected void ddlPartner_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //GetAllAssets(Convert.ToInt32(ddlPartner.SelectedValue), ViewState["SortField"].ToString(), "All");
            //pnlUninsuredAssets.Visible = true;
        }
        ...
        protected void ddlAsset_type_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (objUser.iUser_Type_Id == 1 || objUser.iUser_Type_Id == 2)
            {
                GetAllAssets(Convert.ToInt32(ddlPartner.SelectedValue), ViewState["SortField"].ToString(), ddlAsset_type.SelectedItem.Text);
            }
            else
            {
                GetAllAssets(objUser.iPartner_Id, ViewState["SortField"].ToString(), ddlAsset_type.SelectedItem.Text);
            }
        ...
        protected void btnSendReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objUser.iUser_Type_Id == 1 || objUser.iUser_Type_Id == 2)
                {
                    ds = frmF.Get_All_Assets_Financer_Report_Download(Convert.ToInt32(ddlPartner.SelectedValue));
                    ParnerName = ddlPartner.SelectedItem.Text;
                }
                else
                {
                    ds = frmF.Get_All_Assets_Financer_Report_Download(objUser.iPartner_Id);
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    context.Response.AppendHeader("Content-Disposition", "attachment; filename=" + ParnerName + "_All_Assets_" + DateTime.Now.ToString("dd/MMM/yyyy") + ".csv"); //+ DateTime.Now.ToString("dd/MMM/yyyy HH:mm") 
