...
{
    public partial class ChangeInsuranceCover : System.Web.UI.UserControl
    {
        ...
        protected void btnShowChangeOfCover_Click(object sender, EventArgs e)
        {
            CCom.CurrentUser objUser = new CCom.CurrentUser();
            P.User_Provider uP = new P.User_Provider();
            ...
            if (objUser.iUser_Type_Id == 1 || objUser.iUser_Type_Id == 2)
            {
                GetChangeOfCoverReport(Convert.ToInt32(ddlPartner.SelectedValue));
            }
            else
            {
                GetChangeOfCoverReport(objUser.iPartner_Id);
            }
            lblPeriod.Text = ddlPeriod.SelectedItem.Text + " " + ddlYear.SelectedItem.Text;
            pnlChangeOfCover.Visible = true;
        }
        ...
        protected void btnSendReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                rptChangeOfCover.DataSource = null;
                rptChangeOfCover.DataBind();
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    rptChangeOfCover.DataSource = ds.Tables[0];
                    rptChangeOfCover.DataBind();
                    rptChangeOfCover.Visible = true;
                    rptChangeOfCover.Visible = true;
        ...
        private void GetChangeOfCoverReport(int iPartnerId)
        {
            try
            {
                ...
                P.Report_Provider frmF = new P.Report_Provider();
                DataSet ds = frmF.Get_Asset_ChangeOFCover_By_Financier_By_Period(iPartnerId, Convert.ToInt32(ddlPeriod.SelectedValue), Convert.ToInt32(ddlYear.SelectedValue));
        ...
        private void GetChangeOfCoverAssetHistory(int iAsset_Id, int iAsset_Type_Id)
        {
            try
            {

                rptChangeOfCoverHistory.DataSource = null;
                rptChangeOfCoverHistory.DataBind();

                P.Report_Provider frmF = new P.Report_Provider();
                DataSet ds = frmF.Get_Asset_ChangeOFCover_History(iAsset_Id, iAsset_Type_Id);

                if (ds.Tables[0].Rows.Count > 0)
                {
                    rptChangeOfCoverHistory.DataSource = ds.Tables[0];
                    rptChangeOfCoverHistory.DataBind();
                }
            }
            catch (Exception)
        ...
        protected void rptChangeOfCover_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                if (e.CommandName == "ViewHistory")
                {
                    ...
                    GetChangeOfCoverAssetHistory(Convert.ToInt32(param[0]), Convert.ToInt32(param[1]));
                    pnlChangeOfCover.Visible = false;
                    pnlAllDetails.Visible = true;
                }
            }
        }
