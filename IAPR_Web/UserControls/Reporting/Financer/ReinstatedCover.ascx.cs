...
{
    public partial class ReinstatedCover : System.Web.UI.UserControl
    {
        ...
        public void StartFormLoad()
        {
            ...
            {
                pnlPartner.Visible = true;
                rptUReinstatedCover.DataSource = null;
                rptUReinstatedCover.DataBind();
        ...
        protected void btnSendReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (objUser.iUser_Type_Id == 1 || objUser.iUser_Type_Id == 2)
                {
                    ds = frmF.Get_ReistatedCover_Assets_Financer_Report_Download(Convert.ToInt32(ddlPartner.SelectedValue), Convert.ToInt32(ddlPeriod.SelectedValue), Convert.ToInt32(ddlYear.SelectedValue));
                    ParnerName = ddlPartner.SelectedItem.Text;
                }
                else
                {
                    ds = frmF.Get_ReistatedCover_Assets_Financer_Report_Download(objUser.iPartner_Id, Convert.ToInt32(ddlPeriod.SelectedValue), Convert.ToInt32(ddlYear.SelectedValue));
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    context.Response.AppendHeader("Content-Disposition", "attachment; filename=" + ParnerName + "_Re-instated-Cover_" + ddlPeriod.SelectedItem.Text + "_" + ddlYear.SelectedItem.Text + ".csv"); //+ DateTime.Now.ToString("dd/MMM/yyyy HH:mm") 
        ...
        private void GetNonPaymentReport(int iPartnerId)
        {
            try
            {
                ...
                P.Report_Provider frmF = new P.Report_Provider();
                DataSet ds = frmF.Get_ReistatedCover_Assets_Financer(iPartnerId, Convert.ToInt32(ddlPeriod.SelectedValue), Convert.ToInt32(ddlYear.SelectedValue));

                if (ds.Tables[0].Rows.Count > 0)
                {
                    btnSendReport.Visible = true;
                    rptUReinstatedCover.DataSource = ds.Tables[0];

                    rptUReinstatedCover.DataBind();
                    rptUReinstatedCover.Visible = true;
                    rptUReinstatedCover.Visible = true;
        ...
        protected void rptUReinstatedCover_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
